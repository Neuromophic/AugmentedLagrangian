...
def train_pnn(nn, train_loader, valid_loader, lossfunction, optimizer, args, logger, UUID='default'):
    ...
    for epoch in range(current_epoch, 10**10):
        ...
        if patience > args.PATIENCE: # 500 patience for early-stopping 
            print('Early stop.')
...
def train_pnn_progressive(nn, train_loader, valid_loader, lossfunction, optimizer, args, logger, UUID='default'):
    ...
    for epoch in range(current_epoch, 10**10):
        ...
        if patience_lr > args.LR_PATIENCE: # 100 patience for updating learning rate
            print('lr update')
            lr_update = True
        ...
        if current_lr < args.LR_MIN: # 1e-4 minimal learning rate for stop training
            early_stop = True
            print('early stop.')
            logger.info('Early stop.')
            break
