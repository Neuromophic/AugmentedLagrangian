...
def FT(train_loader, valid_loader, args, msglogger, setup):
    ...
    pnn = torch.load(f'{args.savepath}/pNN_{setup}')
    ...
    optimizer = torch.optim.Adam(pnn.parameters(), lr=args.LR/10.)
    pnn, best = train_pnn_progressive(pnn, train_loader, valid_loader, lossfunction, optimizer, args, msglogger, UUID=setup)
    if best:
        if not os.path.exists(f'{args.savepath}/'):
            os.makedirs(f'{args.savepath}/')
        torch.save(pnn, f'{args.savepath}/pNN_{setup}_FT')
        msglogger.info('Fine tuning if finished.')
    else:
        msglogger.warning('Time out, further training is necessary.') 


if os.path.isfile(f'{args.savepath}/pNN_{setup}_FT'):
    print(f'{setup}_FT exists, skip this training.')
    msglogger.info('Training was already finished.')
elif os.path.isfile(f'{args.savepath}/pNN_{setup}'):
